...
<body>
        ...
        <div id="content">
            ...
            <div style="margin-top: 10px;" class="card">
                <div class="card-body">
                    ...
                    <button id="btn_approved" class="btn btn-success"><i class="fa-solid fa-thumbs-up"></i> Approved</button>
                    <button id="btn_pending" class="btn btn-info"><i class="fa-solid fa-clock"></i> Pending</button>
                    <br><br>
                    <h3>Pending Ingredients</h3>
    ...
    <script type="module">
        ...
        function DisplayItemToTable(value, status) {
            ...
            td4.innerHTML = '<button id="btn_confirm" type="button" class="btn btn-info"><i class="fa-solid fa-check"></i></button>';
            td5.innerHTML = '<button id="btn_edit" type="button" class="btn btn-info"><i class="fa-solid fa-pen-to-square"></i></button>';
            td6.innerHTML = '<button id="btn_delete" type="button" class="btn btn-danger"><i class="fa-sharp fa-solid fa-trash-can"></i></button>';
        ...
        $('#tbody_fa').on('click', 'button#btn_confirm', (event) => {
            ...
            const faRef = ref(db, "Maintenance/Food/Ingredients/" + prev_fa);
            const dataToInsert = {
                value: prev_fa,
                status: 'approved'
            };

            var confirmResult = confirm("Do you want to proceed in approving this ingredient?");
            if (confirmResult) {
                const prevRef = ref(db, "Maintenance/Food/Ingredients/" + prev_fa);
                remove(prevRef).then(() => {
                    console.log("Ingredient Deleted.");
                    set(faRef, dataToInsert).then(() => {
                        alert("Ingredient Approved.");
                        PageRefresh();
                    }).catch((error) => {
        ...
        $('#tbody_fa').on('click', 'button#btn_delete', (event) => {
            ...
            if (confirmResult) {
                const ingRef = ref(db, "Maintenance/Food/Ingredients/" + fa_name);
                remove(ingRef).then(() => {
        ...
        btn_approved.addEventListener("click", () => {
            window.location.href = 'ingredients_list.php';
        });
