...
<head>
    ...
    <title>iLifestylePal - Post Archive</title>
...
<body>
        ...
        <div id="content">
            ...
            <div style="margin-top: 10px;" class="card">
                <div class="card-body">
                    ...
                    <button id="btn_archive" class="btn btn-secondary"><i class="fa-solid fa-folder-open"></i> Archive</button>
                    <br><br>
                    <h3>Post Archive</h3><br>
                    <div style="overflow-x: auto;">
                        <table id="table_archive" class="display" style="width:100%; padding-top: 5px;">
    ...
    <script type="module">
        ...
        var post_no = 0;
        var tbody_archive = document.getElementById("tbody_archive");

        function GetAllPosts() {
            const ArchiveRef = ref(db, "Reports/Archive");
            get(ArchiveRef).then((snapshot) => { 
                const archives = [];
                snapshot.forEach((childSnapshot) => {
                    const archive_id = childSnapshot.key; // Get the key of the post
                    const data = childSnapshot.val(); // Get the data of the post
                    const archive = { archive_id, ...data }; // Create an object that contains both the key and data
                    archives.push(archive); // Push the object to the posts array
                });
                const sortedArchives = archives.sort((a, b) => b.post_timestamp - a.post_timestamp);
                DisplayToTable(sortedArchives);
                $('#table_archive').DataTable(); 
            });
        }

        function DisplayToTable(archives) {
            post_no = 0;
            tbody_archive.innerHTML = "";
            archives.forEach(element => {
                DisplayItemToTable(element.post_uid, element.archive_id, element.post_date, element.post_time, element.post_type, element.post_privacy);
            });
        }

        function DisplayItemToTable(post_uid, archive_id, post_date, post_time, post_type, post_privacy) {
            ...
            tr.appendChild(td6);
            tr.appendChild(td7);
            tbody_archive.appendChild(tr); 
        }

        $('#tbody_archive').on('click', 'button#btn_view', (event) => {
            const parentRow = $(event.currentTarget).closest('tr');
            const rowIndex = $('#table_archive').DataTable().row(parentRow).index();
            const post_id = $('#table_archive').DataTable().cell(rowIndex, 2).data();
            const post_type = $('#table_archive').DataTable().cell(rowIndex, 3).data();
            ...
            const ArchiveRef = ref(db, "Reports/Archive/" + post_id);
            get(ArchiveRef).then((snapshot) => { 
